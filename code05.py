...
class DTree:
    ...
    def train(self, train_data):
        ...
        # 1.若D中实例属于同一类Ck，则T为单节点数，并将类Ck作为节点的类标记，返回T
        if len(y_train.value_counts()) == 1:
            return Node(root=True, label=y_train.iloc[0])

        # 2.若A为空，则T为单节点树，将D中实例树最大的类C可作为该节点的类标记，返回T
        if len(features) == 0:
            return Node(
                root=True,
...
ddd = {'label': None, 'feature': 2, 'tree': {'否': {'label': None, 'feature': 1,
                                                   'tree': {'否': {'label': '否', 'feature': None, 'tree': {}},
